...
注意）リソースURIについて
メディア芸術データベースの事業が文化庁から独立行政法人国立美術館国立アートリサーチセンターに引き継がれましたのでリソース	URIを変更しました。  
2024年11月26日の更新までは、これまで各所で作成されたクエリの動作を考慮し、従来の名前空間（ https://mediaarts-db.bunka.go.jp/ ）を用いていましたが同更新をもって新しいリソースURI（ https://mediaarts-db.artmuseums.go.jp/ ）に変更されました。  
メディア芸術データベースデータセットをご利用くださっている皆様には、この更新に伴って様々な調整が必要になってしまうと思われます。お手間をおかけします。

## 1.概要

### 1.1. データセットの提供元

本データセットは、メディア芸術データベース（以下「MADB」といいます。）<<https://mediaarts-db.artmuseums.go.jp/>>が提供しているメタデータを公開しています。

### 1.2. データセットの権利

メディア芸術データベースデータセットは、自由な二次利用が可能です。ただし、二次利用に際しては、次の事項へのご配慮をお願いいたします。これらのお願いは法的な契約ではありませんが、できる限りご留意の上でご利用くださるよう、ご協力をお願いします。
...
　　dataset/data/配下に登録してあります。更新したものは[Releases](https://github.com/mediaarts-db/dataset/releases)に登録してあります。

### 2.3.　データセットの詳細について

メディア芸術データベースメタデータスキーマ仕様書をご参照ください。
...
|2024年1月31日| ver1.2|メタデータスキーマ仕様書の更新、データセットは以下に順次最新分を掲載 https://github.com/mediaarts-db/dataset/releases |
