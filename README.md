...
注意）リソースURIについて
メディア芸術データベースの事業が文化庁から独立行政法人国立美術館国立アートリサーチセンターに引き継がれましたのでリソース	URIを変更しました。ただし、これまで各所で作成されたクエリの動作を考慮し、現在はまだ従来のもの（ https://mediaarts-db.bunka.go.jp/ ）にしています。数か月程度の一定の期間ののちに新しいリソースURI（ https://mediaarts-db.artmuseums.go.jp/ ）に変更する予定です。日程が決まり次第、アナウンスいたします。
